...
Generate a technical specification document from the inbound source file.
...
Use the matching file from `5 templates/` as the mandatory baseline when one exists. For example:
...
Follow the matched template's design, title pattern, headings, section order, tables, attributes, labels, placeholders, and required fields without fail. Use the inbound file to populate the template, preserve source-specific details, and do not invent unsupported business facts. If a required template section cannot be completed from the source, keep the section in place and mark it as `TBD` or use the template's placeholder guidance.

When a matching template exists, do not use the fallback structure below and do not create a new structure.
...
Return only the completed technical specification in the requested output format.
